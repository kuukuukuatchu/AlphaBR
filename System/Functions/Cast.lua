...
function canCast(SpellID,KnownSkip,MovementCheck)
	...
	if (KnownSkip == true or isKnown(SpellID)) and IsUsableSpell(SpellID) and myCooldown < 0.1
		and (MovementCheck == false or myCooldown == 0 or isMoving("player") ~= true or br.UnitBuffID("player",79206) ~= nil) then
		return true
	end
end
...
function shouldNotOverheal(Unit)
	...
	if br.UnitDebuffID("player",142861) then --Ancient Miasma
		CurShield = select(14,br.UnitDebuffID(Unit,142863)) or select(14,br.UnitDebuffID(Unit,142864)) or select(14,br.UnitDebuffID(Unit,142865)) or (UnitHealthMax(Unit) / 2)
		overheal = 0
	end
		...
		if #lowHPTargets >= NumberOfPlayers then
			...
			if foundTargets ~= nil and #foundTargets >= NumberOfPlayers then
				...
				if math.sqrt(((medX-myX)^2)+((medY-myY)^2)) < 40 then
					br._G.CastSpellByName(GetSpellInfo(SpellID),"target")
...
function castSpell(Unit,SpellID,FacingCheck,MovementCheck,SpamAllowed,KnownSkip,DeadCheck,DistanceSkip,usableSkip,noCast)
	...
	then
		...
		then
			...
			if (br.getSpellCD(SpellID) < select(4,GetNetStats()) / 1000) and (br.getOptionCheck("Skip Distance Check") or br.getDistance("player",Unit) <= spellRange or DistanceSkip == true or inRange(SpellID,Unit)) then
					...
					if timersTable == nil or (timersTable ~= nil and (timersTable[SpellID] == nil or timersTable[SpellID] <= GetTime() -0.6)) then
						if (FacingCheck == true or br.getFacing("player",Unit) == true) and (br.GetUnitIsUnit("player",Unit) or br.units[Unit] ~= nil or br.getLineOfSight("player",Unit) == true) then
							if noCast then
								...
								br._G.CastSpellByName(GetSpellInfo(SpellID),Unit)
...
function castMouseoverHealing(Class)
	if UnitAffectingCombat("player") then
		...
		for i = 1,#SpecialTargets do
			...
			if br.GetUnitExists(target) and not UnitIsPlayer(target) then
...
function isCasting(SpellID,Unit)
	if br.GetUnitIsVisible(Unit) and UnitCastingInfo(Unit) then
...
function br.createCastFunction(thisUnit,debug,minUnits,effectRng,spellID,index,predict,predictPad)
	...
	if (baseSpellID == spellID or overrideSpellID == spellID) and IsUsableSpell(spellID) and not select(2,IsUsableSpell(spellID)) -- Usability Checks
		and br.getSpellCD(spellID) == 0 and (br.getSpellCD(61304) == 0 or select(2,GetSpellBaseCooldown(spellID)) == 0) -- Cooldown Checks
	...
	then
			...
			if ((br.isChecked("Display Failcasts") and not debugOnly) or br.isChecked("Cast Debug")) and debug ~= "debug" then
				...
				elseif debugReason == "Invalid Unit" then
					...
					Print("|cffFF0000Error: |r Failed to cast. - "
							...
							..", Min Range: "..minRange
							..", Max Range: "..maxRange
							..", Distance: "..br.getDistance(targetUnit)
		...
		if thisUnit == "playerGround" or thisUnit == "targetGround" or debug == "groundCC" then
			...
			if br.getDistance(targetUnit) < maxRange or IsSpellInRange(spellName,targetUnit) == 1 then
		...
		if thisUnit == "pettarget" then
			if (br.getDistance(thisUnit,"pet") < maxRange or IsSpellInRange(spellName,thisUnit) == 1) then
		...
		then
			...
			if ((distance >= minRange and distance < maxRange) or IsSpellInRange(spellName,thisUnit) == 1) then
				-- Dead Friend
				if debug == "dead" then
					if UnitIsPlayer(thisUnit) and UnitIsDeadOrGhost(thisUnit) and br.GetUnitIsFriend(thisUnit,"player") then
						return castingSpell(thisUnit,spellID,spellName,icon)
					else
				...
				if (debug == "ground" or debug == "aoe" or debug == "cone" or debug == "rect") then
					if br.isDummy() or isSafeToAoE(spellID,thisUnit,effectRng,minUnits) then
