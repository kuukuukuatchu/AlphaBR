...
-- Default Values
	...
	if not mainButton then mainButton = _G.CreateFrame("Button","MyButtonBR",UIParent,"SecureHandlerClickTemplate") end
	mainButton:SetWidth(br.buttonWidth)
	mainButton:SetHeight(br.buttonHeight)
...
-- Set Main Button
	if br.data.settings[br.selectedSpec] ~= nil and br.data.settings[br.selectedSpec].toggles ~= nil and br.data.settings[br.selectedSpec].toggles['Power'] == 1 then
		mainButton:SetNormalTexture(br.backIconOn)
	else
		mainButton:SetNormalTexture(br.backIconOff)
	end
	...
	mainButton:SetScript("OnEnter", function()
		_G.GameTooltip:SetOwner(mainButton, 0 , 0)
	...
	mainButton:SetScript("OnReceiveDrag", function()
	...
	mainButton:SetScript("OnMouseWheel", function(delta)
	...
	mainButtonFrame = _G.CreateFrame("Frame", nil, mainButton)
	mainButtonFrame:SetWidth(br.data.settings["buttonSize"]*1.67)
	mainButtonFrame:SetHeight(br.data.settings["buttonSize"]*1.67)
	...
	mainButtonFrame.texture:SetTexture(br.genericIconOn)
	br.mainText = mainButton:CreateFontString(nil, "OVERLAY")
