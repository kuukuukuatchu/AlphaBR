...
function pulseNovaDebug()
	...
	function br.ui:createBar(parent, option, unit, tooltip, tooltipSpin)
		...
		if br.data.settings[br.selectedSpec][br.selectedProfile][text .. "Status"] == nil then
			br.data.settings[br.selectedSpec][br.selectedProfile][text .. "Status"] = getHP(unit) or 0
		end
	...
	function createNovaStatusBar(parent, option, x, y, width, heigth)
		...
		if _G[parent .. value .. "Nova"] == nil then
			...
			_G[parent .. value .. "Nova"] = CreateFrame("StatusBar", _G[parent .. value .. "Nova"], UIParent, "BackdropTemplate")
