    ...
    if br.timer:useTimer("debugRestoration", 0.1) then
        ...
        local function castingDPSSpells()
            for i = 1, #dpsSpells do
                if isCastingSpell(dpsSpells[i]) and br.isChecked("Critical HP") and lowest.hp <= getValue("Critical HP") then
                    return true
                end
            end
        end
        ...
        local function actionList_Extras()
            ...
            if ui.checked("DPS Testing") then
                if br.GetObjectExists("target") then
                    if getCombatTime() >= (tonumber(ui.value("DPS Testing")) * 60) and br.isDummy() then
                        StopAttack()
                        ClearTarget()
