...
local function runRotation()
    ...
    local function bestConeHeal(spell, minUnits, health, angle, rangeInfront, rangeAround)
        if not isKnown(spell) or br.getSpellCD(spell) ~= 0 or select(2, IsUsableSpell(spell)) then
            return false
        end
    ...
    local function overhealingcancel()
        ...
        if br.isChecked("Overhealing Cancel") and healing_obj ~= nil then
            if br.getHP(healing_obj) > getValue("Overhealing Cancel") and (isCastingSpell(spell.flashOfLight) or isCastingSpell(spell.holyLight)) then
                SpellStopCasting()
                healing_obj = nil
    ...
    local function SingleTarget()
        ...
        if br.isChecked("LoM after FoL") and isCastingSpell(spell.flashOfLight) and php >= br.getOptionValue("LotM player HP limit") then
