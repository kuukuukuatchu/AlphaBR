			...
			if useInterrupts() then
				for i=1, #enemies.yards20 do
					...
					if canInterrupt(thisUnit,br.getOptionValue("InterruptAt")) then
							-- Quaking Palm
							if br.isChecked("Quaking Palm") and not isCastingSpell(spell.essenceFont) then
								if cast.quakingPalm(thisUnit) then return end
							end
							-- Leg Sweep
							if br.isChecked("Leg Sweep") and not isCastingSpell(spell.essenceFont) then
								if cast.legSweep(thisUnit) then return end
							end
						
						-- Paralysis
						if br.isChecked("Paralysis") and not isCastingSpell(spell.essenceFont) then
		...
		local function dps_actionlist() -- Very secret function for Mistweavers only
			if br.player.ui.mode.dps == 1 then
				if br.friend[1].hp >= getValue("DPS Mode") then
					...
					if br.isChecked("Spinning Crane Kick") and #enemies.yards8 >= 3 and not isCastingSpell(spell.spinningCraneKick) then
					...
					if #enemies.yards10 == 0 and not isCastingSpell(spell.cracklingJadeLightning) and br.isChecked("Crackling Jade Lightning") and not isMoving("player") then
