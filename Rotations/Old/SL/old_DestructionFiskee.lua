    ...
    if br.timer:useTimer("targetMove", 0.8) or combatTime < 0.2 then
      ...
      --Defensive dispel whitelist, value is for range to allies on the unit you are dispelling
        ...
        local function spellUsable(spellID)
          if isKnown(spellID) and not select(2,IsUsableSpell(spellID)) and br.getSpellCD(spellID) == 0 then return true end
          return false
        end
...
--------------------
	-- Action List - Extras
      ...
      if br.isChecked("Shadowfury Hotkey (hold)") and shadowfuryKey and not GetCurrentKeyBoardFocus() then
        ...
        elseif br.getOptionValue("Shadowfury Target") == 3 and isKnown(spell.shadowfury) and br.getSpellCD(spell.shadowfury) == 0 then
