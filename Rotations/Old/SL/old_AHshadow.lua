...
local function runRotation()
    if br.timer:useTimer("debugShadow", 0.1) then --change "debugFury" to "debugSpec" (IE: debugFire)
        ...
        -- Void Bolt
        if br.isValidUnit(units.dyn40) and inCombat and buff.voidForm.exists() and (cd.voidBolt.remain() == 0 or buff.void.exists()) and not isCastingSpell(spell.voidTorrent) and sear == false and not sear == true then
     ...
     --   if sear == true then
      ...
      if sear == true and not cast.current.mindSear() and GetTime() - searCastTime > 1 then sear = false end
        ...
        function actionList_Main()
            ...
            -- VT Refresh 2.0
            if debuff.vampiricTouch.remain("target") < 6.3 and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and vtHPLimit then
            ...
            -- SWP Refresh 2.0
            if debuff.shadowWordPain.remain("target") < 4.8 and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.shadowWordPain.count() <= br.getOptionValue("SWP Max Targets") and swpHPLimit then
            ...
            -- VT Refresh
            if (debuff.vampiricTouch.remain("target") < 6.3 * gcd) or (not debuff.vampiricTouch.exists("target")) and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and vtHPLimit then
                if cast.vampiricTouch() then
                    return
                end
            end
            ...
            -- SWP Refresh
            if (debuff.shadowWordPain.remain("target") < 4.8 * gcd) or (not debuff.shadowWordPain.exists("target")) and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.shadowWordPain.count() < br.getOptionValue("SWP Max Targets") and swpHPLimit then
                if cast.shadowWordPain() then
                    return
                end
            end
            -- Misery + VT APPLY
            if talent.misery and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and not isCastingSpell(spell.vampiricTouch) then
                for i = 1, #enemies.yards40 do
                local thisUnit = enemies.yards40[i]
            ...
            -- No Misery + VT APPLY
            if not talent.misery and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and not isCastingSpell(spell.vampiricTouch) then
                for i = 1, #enemies.yards40 do
                local thisUnit = enemies.yards40[i]
                    if not debuff.vampiricTouch.exists(thisUnit)  then
                        if cast.vampiricTouch(thisUnit, "aoe") then
            ...
            -- AoE + SWP APPLY
            if not talent.misery and debuff.shadowWordPain.count() <= br.getOptionValue("SWP Max Targets") and not isCastingSpell(spell.vampiricTouch) then
                for i = 1, #enemies.yards40 do
                local thisUnit = enemies.yards40[i]
            ...
            -- VT
            if talent.misery and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) then
                for i = 1, #enemies.yards40 do
                    local thisUnit = enemies.yards40[i]
            ...
            -- Mind Sear VF
            if ((mode.rotation == 1 and #enemies.yards10t >= br.getOptionValue("Mind Sear Targets")) or mode.rotation == 2) and charges.shadowWordVoid.count() < 1 and not buff.void.exists() and not moving and not isCastingSpell(spell.mindSear) then
                if cast.mindSear() then
                    return
                end
            end

        --    if not buff.void.exists() and not moving and not isCastingSpell(spell.mindSear) and #enemies.yards10t >= br.getOptionValue("Full Cast Mind Sear") then
          --      for i = 1, #enemies.yards40 do
            --    local thisUnit = enemies.yards40[i]
           ...
           -- end
            ...
            -- Mind Flay 
            if not buff.thoughtsHarvester.exists() and charges.shadowWordVoid.count() < 1 and not buff.void.exists() and not moving and not isCastingSpell(spell.mindFlay) and not isCastingSpell(spell.mindSear) and (mode.rotation == 1 and #enemies.yards10t < br.getOptionValue("Mind Sear Targets")) or (mode.rotation == 3) then
        ...
        function actionList_VoidForm()
            ...
            if (debuff.vampiricTouch.remain("target") < 6.3 * gcd) or (not debuff.vampiricTouch.exists("target")) and vtHPLimit and not buff.void.exists() 
                and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") 
            then
                if cast.vampiricTouch() then
                    return
                end
            ...
            if (debuff.shadowWordPain.remain("target") < 4.8 * gcd) or (not debuff.shadowWordPain.exists("target")) and not buff.void.exists() 
                and not moving and not isCastingSpell(spell.vampiricTouch) and debuff.shadowWordPain.count() <= br.getOptionValue("SWP Max Targets") 
            then
                if cast.shadowWordPain() then
                    return
                end
            ...
            -- VT Tick
            if #enemies.yards40 > 1 and not talent.misery and debuff.vampiricTouch.count() <= br.getOptionValue("VT Max Targets") and not buff.void.exists() and not moving and not isCastingSpell(spell.vampiricTouch) then
                for i = 1, #enemies.yards40 do
                    local thisUnit = enemies.yards40[i]
