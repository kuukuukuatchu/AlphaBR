...
--------------
    ...
    local function castBeam(minUnits, safe, minttd)
        if not isKnown(spell.focusedAzeriteBeam) or br.getSpellCD(spell.focusedAzeriteBeam) ~= 0 then
            return false
        end
    ...
    local function actionList_Defensive()
        if useDefensive() then
            if br.isChecked("Auto Defensive Unavoidables") then
                ...
                if bossID == 126848 and isCastingSpell(256979, boss) and br.GetUnitIsUnit("player", br._G.UnitTarget(boss)) then
