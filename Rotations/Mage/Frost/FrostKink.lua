...
local function runRotation()
   ...
   --[[ if not ropNotice and talent.runeOfPower then
    ...
    local function spellUsable(spellID)
        if isKnown(spellID) and not select(2, IsUsableSpell(spellID)) and br.getSpellCD(spellID) == 0 then
            return true
        end
        return false
    end
    ...
    local function castFrozenOrb(minUnits, safe, minttd)
        if not isKnown(spell.frozenOrb) or br.getSpellCD(spell.frozenOrb) ~= 0 or mode.frozenOrb ~= 1 then
            return false
        end  
    ...
    local function actionList_Extras()
        if br.isChecked("DPS Testing") and br.GetObjectExists("target") and getCombatTime() >= (tonumber(br.getOptionValue("DPS Testing")) * 60) and br.isDummy() then
            StopAttack()
            ClearTarget()
